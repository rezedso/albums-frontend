...
const AlbumDataGrid = ({ albums, list }) => {
  ...
  const handleRemoveAlbumFromList = async (listId, albumId) => {
    await removeAlbumFromList({ listId: list?.id, albumId });
  };
  ...
  const columns = [
    ...
    list
      ? {
          ...
          renderCell: (params) => (
            <IconButton
              onClick={() => handleRemoveAlbumFromList(list?.id, params.row.id)}
            >
              <ClearIcon />
            </IconButton>
          ),
